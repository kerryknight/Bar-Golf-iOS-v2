...
#import "PFRACCallbacks+BGS_Extensions.h"
...
+ (RACSignal *)rac_saveFacebookUserDataToParseForCurrentUser:(id)facebookResult {
    ...
    PFFile *profilePicSmall = facebookResult[kKKUserProfilePicSmallKey];
...
//    DLogPurple(@"fb permissions list: %@", facebookResult);
    ...
    if (facebookEmail && facebookEmail != 0) {
        [[PFUser currentUser] setObject:facebookEmail forKey:kKKUserEmailKey];
    }
    
    if (profilePicSmall) {
        [[PFUser currentUser] setObject:profilePicSmall forKey:kKKUserProfilePicSmallKey];
    }
