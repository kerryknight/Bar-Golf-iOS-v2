...
- (void)saveDisplayNameForNewlySignedUpUser {
    ...
    newUser[kUserDisplayNameKey] = self.displayName.length > 0 ? self.displayName : @"New Player";
...
- (BOOL)displayNameTextLengthIsUnderLimit {
    BOOL underLimit = (self.displayName.length < kMaximumDisplayNameLength);
    
    if (!underLimit) {
        NSString *characterLimit = [NSString stringWithFormat:NSLocalizedString(@"Display names are limited to %i characters", nil), kMaximumDisplayNameLength];
        [(RACSubject *)self.sendErrorSignal sendNext:characterLimit];
    }
...
- (BOOL)isValidDisplayNameLength {
    return (self.displayName.length >= kMinimumDisplayNameLength && self.displayName.length <= kMaximumDisplayNameLength);
}
